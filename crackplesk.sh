...
echo "You don't have to, but if you want to you can donate whmcsbypass.host "
echo "For more information contact piticu81@protonmail.com"
...
PIN="/opt/pleskcrack/pin/pin"
TOOL="/opt/pleskcrack/pleskcrack.so"
...
echo "[+] Activating.."
(cd "$PLESK_HOME/admin/plib/" && eval "$PIN -t $TOOL -logfile /dev/stdout -- $SW_ENGINE_COMMAND $PLESK_HOME/admin/plib/api-cli/license.php -i $KEY")

echo "[+] Replacing sw-engine.service and sw-cp-server.service"
cp -r /opt/pleskcrack/sw-cp-server.service /lib/systemd/system/sw-cp-server.service
cp -r /opt/pleskcrack/sw-engine.service /lib/systemd/system/sw-engine.service
